...
DOC_PATTERN_URL = re.compile(
    r"example|tutorial|doc|api|quickstart|note|guide|blog|using|reference|\.io|sample|demo|template", re.I)
...
def replace_tokens_simple_1(content):
    if type(content) != str:
        print("ERROR: not str")
        content = ""
        return content
    ...
    MAX_API_LENGTH = 150
    # my work
    new_file = DATA_PATH + f"train_code.txt"
    ...
    content = re.sub(r'\S+?((\(\))|(\[\]))\S*|[^,;\.\s]{3,}?\.\S{4,}|\S+?([a-z][A-Z]|[A-Z][a-z]{2,}?)\S*|@\S+|<\S*?>',
                     ' APITAG ', content)
